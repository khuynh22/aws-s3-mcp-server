    ...
    try {
        switch (name) {
            ...
            case "s3_list_objects": {
                ...
                logger.info(
                    { bucket: params.bucket, prefix: params.prefix },
                    "Listing S3 objects"
                );
            ...
            case "s3_presign_put": {
                if (!ALLOW_WRITE) {
                    throw new Error(
                        "Write operations are disabled. Set ALLOW_WRITE=true in .env to enable."
                    );
                }
...
async function main() {
    ...
    logger.info(
        { writeEnabled: ALLOW_WRITE, region: AWS_REGION },
        "Configuration"
    );
